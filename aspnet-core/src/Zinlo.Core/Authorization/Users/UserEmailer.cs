...
namespace Zinlo.Authorization.Users
{
    ...
    /// </summary>
    public class UserEmailer : ZinloServiceBase, IUserEmailer, ITransientDependency
    {
        ...
        public async Task SendInviteUserEmail(string email, int tenantId, string link = null)
        {
            link = link.Replace("{tenantId}", "" + tenantId);
            link = link.Replace("{email}", email);
            link = EncryptQueryParameters(link);
            var tenancyName = GetTenancyNameOrNull(tenantId);
            var emailTemplate = GetTitleAndSubTitle(tenantId, "Invitaion", "Admin invitaion");
            var mailMessage = new StringBuilder();
            mailMessage.AppendLine("<b>" + L("TenancyName") + "</b>: " + tenancyName + "<br />");
            mailMessage.AppendLine(" Please Click on this link to active your account  ");
            mailMessage.AppendLine("<span style=\"font-size: 8pt;\">" + link + "</span>");
            await ReplaceBodyAndSend(email, "Invitaion", emailTemplate, mailMessage);
        }
    }
}
